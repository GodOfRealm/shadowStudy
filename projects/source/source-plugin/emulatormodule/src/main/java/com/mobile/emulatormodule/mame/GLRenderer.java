...
public class GLRenderer implements Renderer {
	...
	public void changedEmulatedSize() {
		Log.v("mm",
				"changedEmulatedSize " + shortBuffer + " "
						+ Emulator.getScreenBuffer());
	...
	public void initVertexes(GL10 gl) {
		...
		float scaleX = (float) Emulator.getWindow_width()
				/ Emulator.getEmulatedWidth();
		float scaleY = (float) Emulator.getWindow_height()
				/ Emulator.getEmulatedHeight();

		float[] coords = {
				// X, Y, Z
				(int) ((float) width * scaleX), 0, 0,
				(int) ((float) width * scaleX),
				(int) ((float) height * scaleY), 0, 0, 0, 0, 0,
				(int) ((float) height * scaleY), 0 };
	...
	public void onDrawFrame(GL10 gl) {
		...
		if (!textureInit) {
			...
			ShortBuffer tmp = ShortBuffer.allocate(getP2Size(Emulator
					.getEmulatedWidth())
			...
			gl.glTexImage2D(GL10.GL_TEXTURE_2D, 0, GL10.GL_RGB,
					...
					getP2Size(Emulator.getEmulatedHeight()), 0, GL10.GL_RGB,
		...
		/*
		 * gl.glTexImage2D(GL10.GL_TEXTURE_2D, 0, GL10.GL_RGB,
		 * Emulator.getEmulatedWidth(),Emulator.getEmulatedHeight(), 0,
		...
		if ((gl instanceof GL11Ext) && !force10) {
			...
			((GL11) gl).glTexParameteriv(GL10.GL_TEXTURE_2D,
					GL11Ext.GL_TEXTURE_CROP_RECT_OES, mCrop, 0);
			((GL11Ext) gl).glDrawTexiOES(0, 0, 0, Emulator.getWindow_width(),
					Emulator.getWindow_height());
		} else {
